...
function toolUI(t){
  switch(t.type){
    case "shadow": return shadowUI(t);
    case "role": return roleUI(t);
    case "pron": return pronUI(t);
    case "letter": return letterUI(t);
    case "srs": return `<p class="mut">下のボタンで復習を開始（SM-2で自動スケジュール）。</p>
      <div class="btns"><button onclick="go('srs');setTimeout(startSrs,50)">▶ 今日の復習を始める</button></div>`;
    case "mock": return mockUI(t);
    default: return freeUI(t);
  }
}
...
function stopTimer(silent){ if(!timer.h)return; clearInterval(timer.h); const m=Math.max(1,Math.round(timer.sec/60)); const tid=timer.id; timer={id:null,sec:0,h:null}; if(!silent){ addField("listen",m); markDone(tid); toast("+"+m+"分 記録しました"); renderToday(); } }
...
function manLog(id,field){ const v=+($("man_"+id).value||0); if(v>0){addField(field,v);markDone(id);toast("+"+v+"分");renderToday();} }

// --- roleplay ---
function roleUI(t){
  const r = C.roleplays[ dnum() % C.roleplays.length ];
  const full = r.prompt+"\n\n"+C.rubric;
  return `<p class="big"><b>今日の場面：${r.title}</b></p>
    ...
    <div class="btns"><button onclick='copyText(${jstr(full)})'>📋 プロンプト＋採点ルーブリックをコピー</button></div>
...
function finishRole(id,defMin){ const m=+($("min_"+id).value||defMin); const d=ensureDay(todayISO());
  d.note=$("note_"+id).value||d.note; const w=$("wpm_"+id).value, f=$("fil_"+id).value;
  if(w!=="")d.wpm=+w; if(f!=="")d.filler=+f; d.ts=Date.now(); save();
  addField("speak",m); markDone(id); toast("+"+m+"分 記録しました"); renderToday(); }
...
// --- OET letter ---
function letterUI(t){
  const L = C.letters[ dnum() % C.letters.length ];
  const d=ensureDay(todayISO());
  return `<p class="big"><b>${L.title}</b></p>
...
function finishLetter(id,defMin){ const d=ensureDay(todayISO()); d.letter=(d.letter||0)+1; d.ts=Date.now(); save(); addField("speak",defMin); markDone(id); toast("紹介状を記録（+"+defMin+"分）"); renderToday(); }
...
function finishMock(id,defMin){ const m=+($("man_"+id).value||defMin); const d=ensureDay(todayISO());
  const sec=$("sec_"+id).value, nt=$("note_"+id).value; if(nt)d.note=(d.note?d.note+" / ":"")+"[模試 "+sec+"] "+nt; d.ts=Date.now(); save();
  addField("listen",m); markDone(id); toast("模試を記録"); renderToday(); }
...
function finishFree(id,defMin){ const m=+($("man_"+id).value||defMin); const d=ensureDay(todayISO());
  const nt=$("note_"+id).value; if(nt)d.note=(d.note?d.note+" / ":"")+nt; d.ts=Date.now(); save();
  addField("listen",m); markDone(id); toast("記録しました"); renderToday(); }
...
function startSrs(all){ const ids = all===true ? DECK.cards.map(c=>c.id) : shuffle([...dueIds(),...newIds()]);
  if(!ids.length){ toast("今日の復習はありません。お疲れさま！"); return; }
  Q={list:ids,i:0}; $("srsHome").style.display="none"; $("srsPlay").style.display="block"; showCard(); go("srs"); $("srsHome").style.display="none"; $("srsPlay").style.display="block"; }
...
function saveManual(){ const k=$("mDate").value||todayISO(); const d=ensureDay(k);
  ...
  d.wpm=$("mWpm").value===""?null:+$("mWpm").value; d.filler=$("mFiller").value===""?null:+$("mFiller").value; d.note=$("mNote").value||""; d.ts=Date.now();
...
function exportJSON(){ const blob=new Blob([JSON.stringify(S,null,2)],{type:"application/json"}); const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="english-canada-"+todayISO()+".json"; a.click(); }
